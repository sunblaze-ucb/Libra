...
def do_addition(fh, ivals, voffset, noconsts):
    for i in range(len(ivals) - 1):
        assert ivals[i] == ivals[i + 1] - 1
    
    fh.write("P V%d = V%d SS V%d E\n" % (voffset, ivals[0], ivals[-1]))
    voffset += 1
...
def get_usage():
    ...
    uStr += " -R        disable randomized zero test        (True)\n\n"

    uStr += " -C        no consts in sub-AC input list      (True)\n"
