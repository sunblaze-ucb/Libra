...
\begin{protocol}
	...
	\begin{itemize}
		\item Define the multilinear extension of array $\mathsf{out}$ as $\tV_0$. $\V$ chooses a random $r_0 \in \mathbb{F}^{s_0}$ and sends it to $\P$. Both parties compute $\tilde{V_0}(r_0)$
		\item For $i=1,...,d$:
		\begin{itemize}
			...
			\item Here we implement the oracle: $\V$ asks the value of $\tilde{V}_i(u), \tilde{V}_i(v)$ from $\P$, and $\V$ compute $\tilde{add}, \tilde{mult}$ locally. 
			\item Then $\V$ combines $\tilde{V_{i}}(R_{u}), \tilde{V_{i}}(R_{v})$ in this way: 
			\begin{itemize}
				\item Let $\gamma: \mathbb{F} \rightarrow \mathbb{F}^{s_i}$ be a line with $\gamma(0)=R_{u}, \gamma(1)=R_{v}$. 
				\item The verifier sends $\gamma(x)$ to $\P$. 
