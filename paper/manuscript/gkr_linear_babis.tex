...
In this section, we present our improvements on the GKR protocol~\cite{GKR} to make the prover run in linear time for arbitrary layered circuits. Before that we present some necessary building blocks.
...
In~\cite{JT_Thesis}, Thaler proposed a dynamic programming algorithm with linear prover time $O(2^\ell)$ for the sumcheck protocol on $\sum\nolimits_{x\in\binary^\ell}f(x)$ where $f$ is a multilinear function on $\ell$ variables. In particular, as described in Section~\ref{subsec::sumcheck}, in the $i$-th round ($i\le \ell$), $\P$ sends $\V$ a univariate polynomial of $x_i$: $$\sum\nolimits_{b_{i+1}, \ldots,b_{\ell, }\in\binary}f(r_1,\ldots, r_{i-1}, x_{i}, b_{i+1},\ldots, b_{\ell})\, ,$$ where $r_1, \ldots, r_{i-1}$ are random values chosen by $\V$ in previous rounds. The degree of the univariate polynomial is 1 as $f$ is multilinear, so it suffices to send two evaluations of the polynomial to $\V$ at points $t = 0$ and $t=1$. 

To compute these evaluations, the idea is that in the $i$-th round, $\P$ stores an array of values $f(r_1,\ldots, r_{i-1}, b_{i}, b_{i+1},\ldots, b_{\ell})$ for all $b_i,\ldots,b_\ell\in\binary$. The size of this array is $2^{\ell-i+1}$. With this array, $f(r_1,\ldots, r_{i-1}, t, b_{i+1},\ldots, b_{\ell})$ for either $t=0$ or $t=1$ can be computed in $O(1)$ (see Step~\ref{alg::dynamic::compute1} of Algorithm~\ref{alg::dynamic}). Therefore the message of the prover 
\[
\sum\nolimits_{b_{i+1},\ldots,b_{\ell, }\in\binary}f(r_1,\ldots, r_{i-1}, t, b_{i+1},\ldots, b_{\ell})
\]
for $t=0$ and $t=1$ can be computed in time $O(2^{\ell-i+1})$ by summing up all the values in the previous step---see Step~\ref{alg::dynamic::sum0}. Finally, the array that will be used in the $(i+1)$-th round can be computed in time $O(2^{\ell-i+1})$ from the array in the $i$-th round (Step~\ref{alg::dynamic::update}).  In this way, the total complexity of generating all the messages in the sumcheck protocol is $O(2^{\ell})+ O(2^{\ell-1}) + O(2^{\ell-2}) + \ldots = O(2^\ell)$. The full protocol is given in Algorithm~\ref{alg::dynamic} (to be compatible with algorithms later, we set also use the value $t=2$ in our evaluations which does not affect the complexity). 

\begin{figure}[t!]
    
	\begin{algorithm}[H]
		\caption{$\{a_1,\ldots,a_\ell \}\leftarrow \textsf{SumCheckMessages}(\textbf{A},\ell)$}\label{alg::dynamic}
		 \textbf{Input:} Array \textbf{A} of $2^\ell$ entries in $\F$ and randomness $r_1,\ldots,r_\ell$;  \\
    \textbf{Output:} 
    Prover messages $\{a_1,\ldots,a_\ell \}$ in sumcheck protocol on $\sum_{x\in\binary^\ell}f(x)$ where $f$ is the multilinear extension of \textbf{A}. Each message $a_i$ consists of 3 elements $(a_{i0},a_{i1},a_{i2})$; 
	...
	\For{$t= 0,1,2$} 				
					\label{alg::dynamic::compute1} \State Compute $f(r_1,\ldots, r_{i-1}, t, b) = \textbf{A}[b]\cdot (1-t)+\textbf{A}[b+2^{\ell-i}]\cdot t$
					\EndFor
...
%\For{$i = 1, \ldots, \ell$} 
				\For{$t\in\{0,1,2\}$}
					\label{alg::dynamic::sum0} \State $a_{it}=\sum_{b\in\binary^{\ell-i}}f(r_1,\ldots, r_{i-1}, t, b)$
...
\begin{figure}[t!]
	...
	\begin{algorithm}[H]
		...
		\begin{algorithmic}[1]
			...
			\State\label{alg::dynamic::updatex} $\textbf{A}[b]=\textbf{A}[b]\cdot(1-r_i)+\textbf{A}[b+2^{2\ell-i}]\cdot r_i$
			\EndFor
			\For{$t\in\{0,1,2\}$} 
