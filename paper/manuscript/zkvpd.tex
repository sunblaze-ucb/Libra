...
\begin{itemize}
			\item \textbf{Perfect Completeness}
				...
				\right]\]
			\item \textbf{Binding}
			For any PPT adversary $\mathcal{A}$ and benign auxilary input $z_1, z_2$ the following probability is negligible of $\lambda$:
			{\footnotesize
			\[\Pr \left[
				\begin{aligned}
				...
				\begin{align*}
					...
					\begin{minipage}{.5\linewidth}
						...
						\begin{enumerate}
							...
							\end{enumerate}
							\item $b \shortrightarrow \A(1^\lambda, \comm, (y_1, ..., y_k, \pi_1, ..., \pi_k), \vp)$
							\item \text{Output b}
						\end{enumerate}
					\end{minipage}
				\end{align*}
