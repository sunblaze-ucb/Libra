...
\begin{itemize}[leftmargin=*]
\item \textbf{libSNARK:} We use jsnark~\cite{jsnark} to write the circuits (rank one constraint system (R1CS)), which compiles them to zero knowledge proofs using the libSNARK backend~\cite{libsnark_impl}. 
...
\item\textbf{Hyrax:} We use the open-source implementation of the system at~\cite{hyrax_impl}.
