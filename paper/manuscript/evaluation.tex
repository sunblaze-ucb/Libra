...
\paragraph{Hardware.} We run all of the experiments on Amazon EC2 c5.9xlarge instances with 70GB of RAM and Intel Xeon platinum 8124m CPU with 3GHz virtual core. Our implementations are not paralleled and we only use a single CPU core in the experiments. We report the average time of 10 executions. %We omit the standard deviation as it is always less than 3\% of the average in our experiments.
...
\hline
\end{tabular}
\caption{\label{tab:GKRCom}Prover time of our linear GKR and previous GKR variants.\yupeng{Jiaheng and Tiancheng, keep 3 or 4 digits for the numbers.}}
\vspace{-0.4in}
}
\end{table}
...
\paragraph{Setup time.} Among all the systems, only \name and libSNARK require trusted setup. Thanks to the optimization described in the beginning of this section, it only takes 202s to generate the public parameters in our largest instance with $n = 2^{24}$. \name only needs to perform this setup once and it can be used for all benchmarks and all circuits with no more inputs. libSNARK requires a per-circuit setup. For example, it takes 1027s for the Merkle tree with 256 leaves, and takes 210s for $64\times 64$ matrix multiplications.
